...
│   │   ├── invest.go           # 持仓/行情/价格历史/收益曲线 + 强制刷新（POST /invest/refresh）
...
└── frontend/                   # React 前端（TypeScript）
    ...
    │   └── check-i18n.mjs      # 三语键集合一致性自检（pre-commit 钩子挂载）
    └── src/
        ├── main.tsx            # 入口
        ├── App.tsx             # 路由
        ...
        └── pages/
            ...
            ├── invest/         # 投资页（持仓表[拖拽排序 + 列头三态排序 + 强制刷新按钮] + 类型筛选 Tab + ¥/$ 切换/交易/曲线/资产占比[同指数分组聚合 + 悬停明细统计]/资产对话框：美股·A股·积存金·中国基金·手动预设）
...
| POST   | /api/invest/refresh               | 强制刷新（绕过行情 60s 与 PE 1h Redis 缓存，拉取全部自动跟踪资产最新价与 PE 并写回；返回 `{refreshed_quotes,total_quotes,refreshed_pes,total_pes}`，上游失败不报错、计数降级） |
...
- **占比分组聚合 + 悬停明细**：资产占比卡按底层指数分组（首批规则：名字含「纳斯达克/Nasdaq」或代码含 QQQ 的资产合并为纳指组，其余独立成行；扩展=规则表加一行）；悬停组行浮层显示各成员占总资产/组内双比例 + 交易统计（N 笔 · 首次~最近日期），浮层恒不含金额（遮蔽态天然安全）
...
- **持仓强制刷新**：持仓表刷新按钮一键绕过行情 60s 与 PE(TTM) 1h 的 Redis 缓存，强制拉取全部自动跟踪资产（Yahoo/积存金/中国基金）最新现价与 PE 并写回资产表；成功 toast 显示刷新计数，持仓/汇总卡/Dashboard 自动更新；上游失败仍走降级链不报错
